...
export const HypercertTile = ({ uri, id }: Omit<Claim, 'creation'> ) => {
  ...
  return (
    <VStack p={4} bgColor={'white'} shadow={'base'} border={'default-grey'} borderRadius={12} justifyContent={'flex-end'} alignItems={'flex-end'} _hover={{backgroundColor:'gray.50'}}>
      ...
      <HStack  justifyContent={'flex-end'}  w={'max'} px={4} py={2} _hover={{background: 'green', fontWeight: 'medium', borderRadius:'full', textColor: 'dark-green'}}>  <Link  href={`https://grants-stack-builder-orpin.vercel.app/#/projects/new?chainId=${chainId}&hypercertId=${id}`} color="dark-green" target={'_blank'}><ExternalLinkIcon boxSize={4}/> Apply</Link></HStack>
      </VStack>
  );
}